...
export default function RespMPage() {
  ...
  const fetchRapports = async () => {
    try {
      ...
      const [rapportRes, tourneeRes, vehiculeRes] = await Promise.all([
        fetch(`/data/rapport.xml?t=${Date.now()}`),
        fetch(`/data/tournee.xml?t=${Date.now()}`),
        fetch(`/data/vehicule.xml?t=${Date.now()}`)
      ]);

      if (!rapportRes.ok) { setRapports([]); return; }
  ...
  useEffect(() => {
    fetchReclamations();
    fetchTrashCans();
    fetchChauffeurs().then(({ chauffeurs, ouvriers, allOuvriers }) => {
      fetchVehicules(chauffeurs).then((vehList) => fetchTournees(vehList, allOuvriers));
    });
    fetchRapports();
    fetchNotifications();
  }, []);
      ...
      case 'modifier-tournee':
        return (
          <ModifierTournee 
            ...
            onCancel={closeModificationPage}
            isEditing={true}
            vehiculeOptions={vehicules.filter(v => (v.disponibilite || '').toLowerCase() === 'disponible')}
            ouvrierOptions={ouvrierOptions}
          />
        );
