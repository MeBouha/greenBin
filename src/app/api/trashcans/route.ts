...
export async function getTrashCansAdresse(adresse: string): Promise<
  Array<{ id: number; adresse: string; lat: number; lng: number; status: 'full' | 'half' | 'empty' }>
> {
  const filePath = path.join(process.cwd(), 'public', 'data', 'trashCan.xml');
  const xml = await fs.readFile(filePath, 'utf8');
  ...
  const filtered = items
    .map((it: any) => {
      const adresseTrash =String(it.lieu.adresse);
      // filtrer par adresse
      if (adresseTrash.toLowerCase() !== adresse.toLowerCase()) return null;
      const id = parseInt(it['@_id'] ?? it.id ?? '0', 10);
      const lat = parseFloat(it.lieu?.coordonnees?.latitude ?? 0);
      const lng = parseFloat(it.lieu?.coordonnees?.longitude ?? 0);
      const status = String(it.status ?? '').toLowerCase();

      return {
        id: Number.isNaN(id) ? 0 : id,
        adresse: adresseTrash,
        lat: Number.isNaN(lat) ? 0 : lat,
        lng: Number.isNaN(lng) ? 0 : lng,
        status,
      };
    })
    .filter(Boolean) as Array<{ id: number; adresse: string; lat: number; lng: number; status: 'full' | 'half' | 'empty' }>; // supprime les nulls
