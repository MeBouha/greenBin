 ...
 */
async function addReport(body: any) {
...
// Get all rapports as raw XML
async function getAllRapports(): Promise<string> {
  try {
...
// Get detailed rapports, optionally filtered by id
async function getRapportsDetailed(id?: number): Promise<any[]> {
  try {
    const xml = await fs.readFile(filePath, 'utf8');
    const rapports: any[] = [];
    ...
    for (const rapportXml of rapportMatches) {
      const rapportId = extractAttr(rapportXml, 'id');
      
      // If id filter is provided, skip non-matching rapports
      if (id && Number(rapportId) !== id) continue;
      ...
      rapports.push({
        id: parseInt(rapportId),
        date,
        tourneeId: parseInt(tourneeId) || parseInt(rapportId),
        chef: null,
        presentEmployees,
        absentEmployees,
...
export async function GET(request: Request) {
  try {
    ...
    if (format === 'detailed') {
      const rapports = await getRapportsDetailed(rapportId ? Number(rapportId) : undefined);
      return NextResponse.json(rapports);
    }
    
    // Otherwise return raw XML data
    const xml = await getAllRapports();
