...
export default function RapportPage() {
  ...
  useEffect(() => {
    async function loadEmployeesForTournee(selectedId?: number | '') {
      try {
        ...
        try {
          const rawChef = sessionStorage.getItem('user');
          if (rawChef) {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    // Calculate metrics based on route distance
    const kilometrage = routeDistance;
    const co2Emis = kilometrage * 1.4;
    const carburantConsomme = kilometrage * 30;

    const payload = { idTournee, vehicule, date, chef, presentEmployees, absentEmployees, selectedTrashcans, kilometrage, co2Emis, carburantConsomme };

    try {
      const res = await fetch('/api/rapports', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      if (!res.ok) {
        ...
        <div style={{display:'flex',flexDirection:'row',gap:20,alignItems:'flex-start'}}>
          ...
          <section style={{flex:2}}>
            <form className="card" onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:12}}>
              ...
              <div>
                <div style={{fontSize:14,fontWeight:600,marginBottom:8}}>Poubelles sélectionnées</div>
              ...
              <div>
                <div style={{fontSize:14,fontWeight:600,marginBottom:8}}>Données de route</div>
                <div style={{display:'flex',flexDirection:'column',gap:8}}>
                  <div>
                    <label style={{display:'block',fontSize:13,marginBottom:4}}>Kilométrage (km)</label>
                    <input className="input" type="number" step="0.01" value={routeDistance} onChange={e=>setRouteDistance(Number(e.target.value) || 0)} readOnly style={{backgroundColor:'#f3f4f6'}} />
                  </div>
                  <div>
                    <label style={{display:'block',fontSize:13,marginBottom:4}}>CO₂ Émis (kg)</label>
                    <input className="input" type="number" step="0.01" value={(routeDistance * 1.4).toFixed(2)} readOnly style={{backgroundColor:'#f3f4f6'}} />
                  </div>
                  <div>
                    <label style={{display:'block',fontSize:13,marginBottom:4}}>Carburant Consommé (L)</label>
                    <input className="input" type="number" step="0.01" value={(routeDistance * 30).toFixed(2)} readOnly style={{backgroundColor:'#f3f4f6'}} />
                  </div>
                </div>
              </div>
          ...
          <aside style={{flex:1}} className="card">
            ...
            <div style={{marginTop:12}}>
              <div style={{fontWeight:600,marginBottom:6}}>Métriques de route</div>
              <div style={{display:'flex',flexDirection:'column',gap:6,fontSize:13}}>
                <div>Kilométrage: <strong>{routeDistance.toFixed(2)} km</strong></div>
                <div>CO₂ Émis: <strong>{(routeDistance * 1.4).toFixed(2)} kg</strong></div>
                <div>Carburant: <strong>{(routeDistance * 30).toFixed(2)} L</strong></div>
              </div>
            </div>
          </aside>
